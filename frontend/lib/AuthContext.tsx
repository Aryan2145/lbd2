...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Persist a session from an already-issued token + user. Used by login() and
  // by the verify-email / reset-password flows, which log the user in directly.
  ...
  return <Ctx.Provider value={{ token, user, login, setSession, logout }}>{children}</Ctx.Provider>;
}
